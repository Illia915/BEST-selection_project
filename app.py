...
# ── Мова ──────────────────────────────────────────────────────────────────────
lang = st.sidebar.radio('', ['en', 'uk'], horizontal=True, key='lang',
                        format_func=lambda x: '🇬🇧 EN' if x == 'en' else '🇺🇦 UA')
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
st.sidebar.markdown(f'<div class="section-label">{t("sidebar_data_source", lang)}</div>',
                    unsafe_allow_html=True)

uploaded = st.sidebar.file_uploader(
    t('sidebar_upload_label', lang),
    type=['BIN', 'bin'],
...
if uploaded is None:
    ...
    if os.path.exists(data_dir):
        ...
        if bin_files:
            st.sidebar.markdown(
                f'<div class="section-label" style="margin-top:12px">{t("sidebar_sample_logs", lang)}</div>',
                unsafe_allow_html=True)
...
st.sidebar.markdown(
    f'<div class="section-label" style="margin-top:16px">{t("sidebar_visualization", lang)}</div>',
    unsafe_allow_html=True)
color_by = st.sidebar.radio(
    t('sidebar_color_label', lang),
    ['speed', 'time'],
    format_func=lambda x: t('sidebar_color_speed', lang) if x == 'speed' else t('sidebar_color_time', lang),
    label_visibility='collapsed',
)

st.sidebar.markdown(
    f'<div class="section-label" style="margin-top:16px">{t("sidebar_ai_engine", lang)}</div>',
    unsafe_allow_html=True)
gemini_key = st.sidebar.text_input(
    'Gemini API Key',
    type='password',
    placeholder=t('sidebar_api_key_placeholder', lang),
    help=t('sidebar_api_key_help', lang),
    label_visibility='collapsed',
)
...
ai_mode = st.sidebar.radio(
    'Mode',
    ['single', 'ab'],
    format_func=lambda x: t('sidebar_mode_single', lang) if x == 'single' else t('sidebar_mode_ab', lang),
    label_visibility='collapsed',
)
...
if uploaded is not None or demo_path:
    ...
    st.markdown(
        ...
        f'<div class="section-label" style="margin-bottom:0">{t("section_metrics", lang)}</div>'
    ...
    with tab_ai:
        mode_label = (
            f'{t("ai_single_caption", lang)} · <span class="model-badge">{selected_model}</span>'
            if ai_mode == 'single'
            else f'{t("ai_ab_caption", lang)} · <span class="model-badge">{len(ab_models or [])} {t("ai_models_label", lang)}</span>'
        )
        st.markdown(
            f'<div style="font-size:13px;color:#8b949e;margin-bottom:16px">{mode_label}</div>',
            unsafe_allow_html=True)
        ...
        with col_info:
            st.markdown(
                f'<div style="font-size:12px;color:#8b949e;padding-top:8px">{t("ai_info", lang)}</div>',
                unsafe_allow_html=True)

        if run_ai:
            if not gemini_key:
                st.warning(t('ai_warn_no_key', lang))
            elif ai_mode == 'ab':
                if not ab_models:
                    st.warning(t('ai_warn_no_models', lang))
                else:
                    ...
                    for col, res in zip(cols, results):
                        with col:
                            ...
                            st.download_button(
                                f'{t("ai_export_ab", lang)} ({res["model"]})',
                                data=res['text'],
        ...
        if usage['requests'] > 0:
            st.markdown(f"""
            <div class="token-bar">
                <div class="token-stat">{t('token_requests', lang)} <span>{usage['requests']}</span></div>
                <div class="token-stat">{t('token_total', lang)} <span>{usage['total_tokens']}</span></div>
                <div class="token-stat">{t('token_prompt', lang)} <span>{usage['prompt_tokens']}</span></div>
                <div class="token-stat">{t('token_completion', lang)} <span>{usage['completion_tokens']}</span></div>
            </div>
            """, unsafe_allow_html=True)
    ...
    <div class="feature-grid">
        <div class="feature-card">
            <div class="feature-card-title">{t('landing_feat_3d_title', lang)}</div>
            <div class="feature-card-desc">{t('landing_feat_3d_desc', lang)}</div>
        </div>
        <div class="feature-card">
            <div class="feature-card-title">{t('landing_feat_metrics_title', lang)}</div>
            <div class="feature-card-desc">{t('landing_feat_metrics_desc', lang)}</div>
        </div>
        <div class="feature-card">
            <div class="feature-card-title">{t('landing_feat_map_title', lang)}</div>
            <div class="feature-card-desc">{t('landing_feat_map_desc', lang)}</div>
        </div>
        <div class="feature-card">
            <div class="feature-card-title">{t('landing_feat_ai_title', lang)}</div>
            <div class="feature-card-desc">{t('landing_feat_ai_desc', lang)}</div>
        </div>
        <div class="feature-card">
            <div class="feature-card-title">{t('landing_feat_ab_title', lang)}</div>
            <div class="feature-card-desc">{t('landing_feat_ab_desc', lang)}</div>
        </div>
        <div class="feature-card">
            <div class="feature-card-title">{t('landing_feat_log_title', lang)}</div>
            <div class="feature-card-desc">{t('landing_feat_log_desc', lang)}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
